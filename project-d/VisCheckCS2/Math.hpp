...
inline Vector3 VectorSub(const Vector3& lhs, const Vector3& rhs)
{
    return { lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z };
}

inline float VectorDot(const Vector3& lhs, const Vector3& rhs)
{
    return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z;
}

inline Vector3 VectorCross(const Vector3& lhs, const Vector3& rhs)
{
    return {
...
struct AABB {
    ...
    bool RayIntersects(const Vector3& rayOrigin, const Vector3& rayDir) const {
        ...
        for (int i = 0; i < 3; ++i) {
            const float dirComponent = rayDirArr[i];
            if (std::abs(dirComponent) < 1e-8f)
            {
                if (rayOriginArr[i] < minArr[i] || rayOriginArr[i] > maxArr[i])
                    return false;
                continue;
            }
